    ...
    try {
        ...
        for ( let i = 0; i < data.length; i++) {
            const ArrayData = {
                ...
                AvailableAt :  data[i].available_at,
                ID: data[i].id,
            };
            ...
            if ( ! existingProfile) {
                ...
                await hanldeUpdateDetailProfile(data[i].id, res);
...
const hanldeUpdateDetailProfile = async (ID, res) => {
    const response = await fetch(
        "https://api.apimo.pro/agencies/22617/properties/"+ID,
        {
            method: "GET",
            headers: new Headers({
                Authorization:
    ...
    if ( results ) {
        //check if the profile exists
        const existingProfile = await ProfilesModel.findOne({ ID: ID });

        if( ! existingProfile)  {
            return res.status(401).json({ error: "Profile Invalid in DB" });
        }
        ...
        await ProfilesModel.updateOne(
            { ID: ID },
            {
                ...
                Proximities: results.proximities,
